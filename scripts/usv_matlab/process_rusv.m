...
function handles=load_wavfiles(handles, path)
    ...
    if (length(filelist1)+length(filelist2)) > 0,
        ...
        handles.datetime = char (datetime("now", 'Format', '_MMdyHHmmss' ));
        handles.audiodir = strcat(handles.audiodir, handles.datetime);
...
function [syllable_data, syllable_stats, filestats, fs] = syllable_activity_file_stats(handles, audiofile, TotNbFrames, syllable_data, syllable_use, Nfft)
    ...
    % start processing
    %fprintf("Syllable data: ", syllable_use)
...
function write_syllables(handles, syllable_ndx)
    ...
    %fprintf("syllable_patch_fft_dB before resize \n", syllable_patch_fft_dB);
    syllable_patch_fft_dB = resizem(syllable_patch_fft_dB,[handles.patch_window, handles.patch_window], 'bilinear');

    %fprintf('patch window size \n', [handles.patch_window, handles.patch_window]);

    [pathstr, name, ext] = fileparts(handles.filename);
    name = strcat(name, handles.datetime);
